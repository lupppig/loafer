...
class TestGeminiProviderTransform:
    ...
    @patch("loafer.llm.gemini.genai")
    def test_successful_response(self, mock_genai: MagicMock) -> None:
        code = "def transform(data):\n    return data\n"
        fake_model = MagicMock()
        fake_model.generate_content.return_value = _make_response(code)
        mock_genai.GenerativeModel.return_value = fake_model

        provider = GeminiProvider(api_key="test-key")
        result = provider.generate_transform_function(schema_sample={"col": {}}, instruction="noop")
    ...
    @patch("loafer.llm.gemini.genai")
    def test_response_with_markdown_fences(self, mock_genai: MagicMock) -> None:
        raw = "```python\ndef transform(data):\n    return data\n```"
        fake_model = MagicMock()
        fake_model.generate_content.return_value = _make_response(raw)
        mock_genai.GenerativeModel.return_value = fake_model
    ...
    @patch("loafer.llm.gemini.genai")
    def test_empty_response_raises(self, mock_genai: MagicMock) -> None:
        resp = MagicMock()
        resp.text = ""
        resp.usage_metadata = None
        fake_model = MagicMock()
        fake_model.generate_content.return_value = resp
        mock_genai.GenerativeModel.return_value = fake_model
    ...
    @patch("loafer.llm.gemini.genai")
    def test_rate_limit_raises_after_retries(self, mock_genai: MagicMock) -> None:
        fake_model = MagicMock()
        fake_model.generate_content.side_effect = Exception("429 Resource has been exhausted")
        mock_genai.GenerativeModel.return_value = fake_model
        ...
        assert fake_model.generate_content.call_count == 4
...
class TestGeminiProviderEltSql:
    ...
    @patch("loafer.llm.gemini.genai")
    def test_successful_sql_response(self, mock_genai: MagicMock) -> None:
        sql = "SELECT id, LOWER(name) AS name FROM users_raw"
        fake_model = MagicMock()
        fake_model.generate_content.return_value = _make_response(sql)
        mock_genai.GenerativeModel.return_value = fake_model
    ...
    @patch("loafer.llm.gemini.genai")
    def test_sql_fences_stripped(self, mock_genai: MagicMock) -> None:
        raw = "```sql\nSELECT 1 FROM t\n```"
        fake_model = MagicMock()
        fake_model.generate_content.return_value = _make_response(raw)
        mock_genai.GenerativeModel.return_value = fake_model
